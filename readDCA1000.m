...
function [retVal] = readDCA1000(folder_locaion, numADCSamples, RX, DopplerIndex)
...
temp = [];
for j = 1:numChirps
    temp = [temp, adcData(RX, DopplerIndex + numADCSamples * (j-1))];
end

WIN = 18;
fsam = 100 / 3;
FFTN = 2^16;
[S,F,T,P] = spectrogram(temp,WIN,WIN-1,FFTN,fsam);
f_axis=linspace(fsam/2,-fsam/2,FFTN);
figure;
collect1 = imagesc(T,f_axis,abs(fftshift(S,1)));
xlabel('Time(s)');ylabel('Doppler frequency(Hz)');
set(gca,'FontSize',28,'Fontname', 'Times New Roman');
set(gcf,'unit','centimeters','position',[1 1 20 16])%调整窗口位置 和 图片坐标轴的大小
set(gca,'Position',[0.17 0.17 .8 .8]); %调整图片在figure区的位置
